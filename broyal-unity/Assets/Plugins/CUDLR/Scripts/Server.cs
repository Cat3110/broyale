...
namespace CUDLR {
  ...
  public class Server : MonoBehaviour {
    ...
    public virtual void Awake() {
      ...
      fileRoot = Path.Combine(Application.streamingAssetsPath, nameof(CUDLR));
    ...
    private void RegisterRoutes() {
      ...
      foreach(Type type in Assembly.GetExecutingAssembly().GetTypes()) {
        ...
        foreach(MethodInfo method in type.GetMethods(BindingFlags.Public|BindingFlags.Static)) {
          ...
          if (cbm == null) {
            Debug.LogError($"Method {type}.{method.Name} takes the wrong arguments for a console route.");
            continue;
          }
          ...
          foreach(RouteAttribute route in attrs) {
            if (route.m_route == null) {
              Debug.LogError($"Method {type}.{method.Name} needs a valid route regexp.");
              continue;
            }
    ...
    static void RegisterFileHandlers() {
      string pattern = $"({string.Join("|", fileTypes.Select(x => x.Key).ToArray())})";
      var downloadRoute = new RouteAttribute($@"^/download/(.*\.{pattern})$");
            RouteAttribute fileRoute = new RouteAttribute($@"^/(.*\.{pattern})$");
