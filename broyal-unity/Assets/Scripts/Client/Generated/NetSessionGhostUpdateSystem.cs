...
public class NetSessionGhostUpdateSystem : JobComponentSystem
{
    [BurstCompile]
...
#pragma warning restore 649
#endif
        [ReadOnly] public ArchetypeChunkBufferType<NetSessionSnapshotData> ghostSnapshotDataType;
        [ReadOnly] public ArchetypeChunkEntityType ghostEntityType;
        public ArchetypeChunkComponentType<NetSession> ghostNetSessionType;
        ...
        public void Execute(ArchetypeChunk chunk, int chunkIndex, int firstEntityIndex)
        {
            ...
            var ghostNetSessionArray = chunk.GetNativeArray(ghostNetSessionType);
#if UNITY_EDITOR || DEVELOPMENT_BUILD
            ...
            for (int entityIndex = 0; entityIndex < ghostEntityArray.Length; ++entityIndex)
            {
                ...
                NetSessionSnapshotData snapshotData;
                if (!snapshot.GetDataAtTick(targetTick, targetTickFraction, out snapshotData))
                    return;

                var ghostNetSession = ghostNetSessionArray[entityIndex];
                ghostNetSession.TimeToStart = snapshotData.GetNetSessionTimeToStart(deserializerState);
                ghostNetSessionArray[entityIndex] = ghostNetSession;
            }
        }
    }
    [BurstCompile]
...
#pragma warning restore 649
    ...
    protected override void OnCreate()
    {
        ...
        {
            All = new []{
                ComponentType.ReadWrite<NetSessionSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
                ComponentType.ReadWrite<NetSession>(),
            },
        ...
        {
            All = new []{
                ComponentType.ReadOnly<NetSessionSnapshotData>(),
                ComponentType.ReadOnly<GhostComponent>(),
                ComponentType.ReadOnly<PredictedGhostComponent>(),
                ComponentType.ReadWrite<NetSession>(),
            }
        });
        RequireForUpdate(GetEntityQuery(ComponentType.ReadWrite<NetSessionSnapshotData>(),
            ComponentType.ReadOnly<GhostComponent>()));
    }
    protected override JobHandle OnUpdate(JobHandle inputDeps)
    {
        ...
        if (!m_predictedQuery.IsEmptyIgnoreFilter)
        {
            var updatePredictedJob = new UpdatePredictedJob
            {
                ...
                ghostSnapshotDataType = GetArchetypeChunkBufferType<NetSessionSnapshotData>(true),
                ...
                ghostNetSessionType = GetArchetypeChunkComponentType<NetSession>(),
...
public partial class NetSessionGhostSpawnSystem : DefaultGhostSpawnSystem<NetSessionSnapshotData>
{
}
